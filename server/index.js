...
app.post("/send-game-invite", async (req, res) => {
    try {
        ...
        const toUser = await User.findOne({ email: toEmail });
        if (!fromUser || !toUser) return res.status(404).json("User not found");
        if (toUser.gameRequests.includes(fromUser._id)) return res.status(400).json("Invite already sent");
        toUser.gameRequests.push(fromUser._id);
        await toUser.save();
...
app.post("/accept-game-invite", async (req, res) => {
    try {
        ...
        const requester = await User.findOne({ email: requesterEmail });
        if (!user || !requester) return res.status(404).json("User not found");
        user.gameRequests = user.gameRequests.filter(id => id.toString() !== requester._id.toString());
        await user.save();
...
app.get("/game-invites/:email", async (req, res) => {
    try {
        const user = await User.findOne({ email: req.params.email }).populate("gameRequests", "username email");
        if (!user) return res.status(404).json("User not found");
        res.status(200).json(user.gameRequests);
    } catch (err) { res.status(500).json("Server error"); }
});
...
app.post("/start-game", async (req, res) => {
    try {
        ...
        if (mode === "bot_chase") currentTurn = "Bot";
        else if (mode === "bot_target") { currentTurn = player1; target = 50; }
        
        const newGame = new Game({
            player1, player2, mode, currentTurn, status,
            target,
            isTossCompleted: true, // Auto complete toss for bot games
            lastPlay: { turnCount: 0 }
        });
        await newGame.save();
        res.status(200).json({ gameId: newGame._id });
...
app.post("/play-live-turn", async (req, res) => {
    try {
        ...
        if (game.player2 === "Bot") {
            // Auto generate bot move if P1 moved
            if (game.p1Move !== null) {
                // If P1 timed out, bot plays safe.
                game.p2Move = game.p1Move === 'timeout' ? 1 : generateBotMove(game, game.p1Move);
            }
        }
